...
IMPLS = ada awk bash basic c chuck clojure coffee common-lisp cpp crystal cs d dart \
	...
	haxe hy io java js julia kotlin livescript logo lua make mal matlab micropython \
	miniMAL nasm nim objc objpascal ocaml perl perl6 php pil plpgsql plsql \
	powershell ps python r racket rexx rpython ruby rust scala scheme skew swift \
	swift3 tcl ts vb vhdl vimscript yorick
...
micropython_STEP_TO_PROG = micropython/$($(1)).py
